...
class _RegisterLandPageState extends State<RegisterLandPage> {
  ...
  final LandRegistrationService _registrationService = LandRegistrationService();
  ...
  Future<void> _getAddressFromLatLng(LatLng location) async {
    try {
      setState(() {
        _locationController.text = "Location at ${location.latitude.toStringAsFixed(6)}, ${location.longitude.toStringAsFixed(6)}";
      });
    } catch (e) {
  ...
  Future<void> _evaluateLand() async {
    ...
    try {
      ...
      final result = await _apiService.estimateLandValue(
        ...
        utilities: (_amenities['electricity'] ?? false) && (_amenities['water'] ?? false),
      );
  ...
  void _simulateEvaluation() {
    ...
    switch (_selectedLandType) {
      case 'RESIDENTIAL': typeMultiplier = 1.5; break;
      case 'COMMERCIAL': typeMultiplier = 2.0; break;
      case 'INDUSTRIAL': typeMultiplier = 1.8; break;
      case 'AGRICULTURAL': typeMultiplier = 1.0; break;
    }
    ...
    final estimatedValue = (surfaceArea * basePrice * typeMultiplier * amenitiesMultiplier * 3000).round(); // Convert to TND
    final estimatedValueEth = surfaceArea * basePrice * typeMultiplier * amenitiesMultiplier;
    
    // Create location info
    final locationInfo = LocationInfo(
      position: _selectedLocation ?? LatLng(36.8065, 10.1815), // Default to Tunis
    ...
    _showSnackBar('Using simulated evaluation (API unavailable)', isError: false);
  }
...
Future<void> _submitRegistration() async {
  ...
  if (!_hasEvaluated || !_isAcceptingPrice) {
    ...
    _showSnackBar('Please evaluate and accept the price first', isError: true);
    return;
  }
  ...
  try {
    ...
    final pricePerToken = _evaluationResult!.valuation.estimatedValueETH != null
        ? _evaluationResult!.valuation.estimatedValueETH! / double.parse(_totalTokensController.text)
        : _evaluationResult!.valuation.estimatedValue / double.parse(_totalTokensController.text) / 3000;
    ...
    final result = await _registrationService.registerLand(
      ...
      images: [], // If you have a separate images list
          ...
          content: SingleChildScrollView(
            child: Column(
              ...
              children: [
                ...
                SizedBox(height: 8),
                _buildCorsFixItem('1. Ensure your NestJS backend has CORS properly enabled:'),
                Padding(
                ...
                SizedBox(height: 12),
                _buildCorsFixItem('2. Make sure the server responds to OPTIONS preflight requests'),
                SizedBox(height: 12),
                _buildCorsFixItem('3. For development, try running Chrome with web security disabled:'),
                Padding(
                  padding: EdgeInsets.only(left: 30),
                  child: Text(
                ...
                SizedBox(height: 12),
                _buildCorsFixItem('4. Check the browser console for detailed errors:'),
                Padding(
                  padding: EdgeInsets.only(left: 30),
                  child: Text(
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () {
                ...
                Navigator.of(context).pop();
                _showSuccessDialog('test-land-${DateTime.now().millisecondsSinceEpoch}');
              },
           ...
           return Expanded(
             child: Text(
               ...
               style: TextStyle(
                 ...
                 color: index == _currentStep ? AppColors.primary : Colors.grey[600],
               ),
             ),
           );
         }),
             ...
             children: [
               GoogleMap(
                 ...
                 initialCameraPosition: CameraPosition(
                   target: _selectedLocation ?? LatLng(36.8065, 10.1815), // Default to Tunis
                   zoom: 14,
                 ),
                 onTap: _onMapTap,
                 markers: _markers,
 ...
 Widget _buildAmenitySection(String title, IconData icon, List<String> amenities) {
   return Card(
     elevation: 0,
     shape: RoundedRectangleBorder(
       borderRadius: BorderRadius.circular(12),
...
Container(
 ...
 Widget _buildDocumentItem(PlatformFile file) {
   IconData fileIcon;
   String fileType;
   Color fileColor;
         ...
         Expanded(
           child: Column(
             ...
             children: [
               ...
               Text(
                 '$fileType • ${_formatFileSize(file.size)}',
                 style: TextStyle(
                   fontSize: 14,
                   color: Colors.grey[600],
                 ),
     ...
     children: [
       ...
       if (_ethPriceData != null) 
         Container(
           ...
           child: Row(
             children: [
               Icon(Icons.currency_exchange, color: Colors.blue[700], size: 28),
               SizedBox(width: 16),
               Expanded(
                 child: Column(
                   crossAxisAlignment: CrossAxisAlignment.start,
           ...
           children: [
             ...
             TextFormField(
               ...
               decoration: InputDecoration(
                 ...
                 helperText: 'How many tokens do you want to create for your land?',
               ),
               keyboardType: TextInputType.number,
               validator: (value) {
                 if (value == null || value.isEmpty) {
 ...
 Widget _buildValuationResultDisplay() {
   ...
   final ethPrice = valuation.currentEthPriceTND ?? (_ethPriceData != null ? _ethPriceData!['ethPriceTND'] : 3000.0);
         ...
         Row(
           children: [
             ...
             Expanded(
               child: _buildEvalDetailCard(
                 'Price Per Token (TND)',
                 NumberFormat('#,###').format(valuation.estimatedValue / double.parse(_totalTokensController.text)),
                 Icons.price_check,
                 Colors.green,
               ),
             ),
         ...
         Row(
           children: [
             Expanded(
               child: _buildEvalDetailCard(
                 'Price Per Token (ETH)',
                 ((valuation.estimatedValueETH ?? valuation.estimatedValue / ethPrice) / 
                   double.parse(_totalTokensController.text)).toStringAsFixed(6),
 ...
 Widget _buildEvalDetailCard(String label, String value, IconData icon, Color color) {
   return Container(
     padding: EdgeInsets.all(20),
     decoration: BoxDecoration(
       color: Colors.white,
       ...
       children: [
         ...
         _buildReviewSection(
           ...
           items: [
             ...
             {'label': 'Description', 'value': _descriptionController.text.isNotEmpty ? _descriptionController.text : 'Not provided'},
             {'label': 'Location', 'value': _locationController.text},
             {'label': 'Surface Area', 'value': '${_surfaceController.text} sq m'},
             {'label': 'Land Type', 'value': _selectedLandType ?? 'Not specified'},
         ...
         _buildReviewSection(
           ...
           items: [
             ...
             {'label': 'Price Per Token', 'value': '${pricePerToken.toStringAsFixed(6)} ETH'},
             {'label': 'Total Land Value (TND)', 'value': '${NumberFormat('#,###').format(_evaluationResult!.valuation.estimatedValue)}'},
             {'label': 'Total Land Value (ETH)', 'value': '${ethValue.toStringAsFixed(4)}'},
           ],
         ),
         SizedBox(height: 24),
         ...
         _buildReviewSection(
           ...
           items: [
             {'label': 'Documents Uploaded', 'value': '${_documents.length} files'},
           ],
         ),
         SizedBox(height: 32),
         ...
         Container(
           ...
           decoration: BoxDecoration(
             ...
             borderRadius: BorderRadius.circular(16),
             border: Border.all(
               color: _isAcceptingPrice ? Colors.green[300]! : Colors.amber[300]!,
             ),
           ),
           child: Column(
             crossAxisAlignment: CrossAxisAlignment.start,
             children: [
               Row(
                 children: [
                   Icon(
                     _isAcceptingPrice ? Icons.check_circle : Icons.info_outline,
                     color: _isAcceptingPrice ? Colors.green[700] : Colors.amber[800],
                     size: 28,
                   ),
                   SizedBox(width: 12),
                   Text(
                     ...
                     style: TextStyle(
                       fontSize: 20,
                       fontWeight: FontWeight.bold,
                       color: _isAcceptingPrice ? Colors.green[800] : Colors.amber[900],
                     ),
                   ),
                 ],
               ),
 ...
 Widget _buildReviewSection({required String title, required IconData icon, required List<Map<String, String>> items}) {
   ...
   return Row(
     ...
     children: [
       ...
       if (_currentStep < _steps.length - 1)
         ElevatedButton.icon(
           onPressed: () {
             ...
             if (_currentStep == 0) {
               ...
               if (_locationController.text.isNotEmpty && _selectedLocation != null) {
                 setState(() {
                   _currentStep++;
                 });
               } else {
                 _showSnackBar('Please enter a location and select a position on the map', isError: true);
               }
               ...
               if (_documents.isEmpty) {
                 _showSnackBar('Please upload at least one document', isError: true);
               } else {
                 setState(() {
                   _currentStep++;
                 });
               ...
               if (!_hasEvaluated) {
                 _showSnackBar('Please evaluate your land before proceeding', isError: true);
               } else {
                 setState(() {
                   _currentStep++;
                 });
         ...
         ElevatedButton.icon(
           onPressed: !_isAcceptingPrice || _isLoading ? null : _submitRegistration,
